...
//List.cpp
//09/11/2018
//description: debugging debug.cpp with llbd
...
int main() {
    ...
    for ( nCount = 0; nCount < MAX; nCount++ ) {
        ...
        cin >> nValues[nCount]; //replace 1 with ncount, will take consideration of next inputs
    }

    // Now we echo the input back to the user
    for ( nCount = 0; nCount < MAX; nCount++) { //start at 0 instead of 1
...
double GetAverage(int nIn[], int nMax) {
    ...
    double temp = 0.0;
    for ( int i = 0; i < nMax; i++) //change to <, 0 should be incrementing up
        temp += nIn[i];
    temp /= nMax;
